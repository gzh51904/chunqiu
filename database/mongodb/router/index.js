...
Router.use((req,res,next)=>{
    ...
    // if(has){
        ...
        res.header("Access-Control-Allow-Headers", "Content-Type,Content-Length, token, Accept,X-Requested-With,Authorization");
...
Router.use('/verify', (req, res) => {
    ...
    // 成功：放行
    // 失败：
    let tk = req.headers.authorization; console.log('mytoken:0--------------', tk)
    if (!token.jiemi(tk)) {
        res.send(formatdata({ status: 401, msg: 'unauthorized' }))
    } else {
        res.send(formatdata())
